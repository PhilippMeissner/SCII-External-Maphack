namespace PatternScanner
{
	...
	public class PatternScan
	{
		...
		public uint SelectionFunction()
		{
			...
			{
				byte[] bPattern = new byte[] { 0x55, 0x8B, 0xEC, 0xB8, 0, 0, 0, 0, 0xE8, 0, 0, 0, 0, 0x8A, 0x45, 0x1C, 0x56, 0x8B, 0x75, 0x18, 0x56, 0xE8, 0, 0, 0, 0, 0x84, 0xC0 }; //Pattern valid as of 2.1.7
		...
		public uint OrderFunction()
		{
			...
			{
				byte[] bPattern = new byte[] { 0x80, 0x3D, 0, 0, 0, 0, 0x00, 0x57, 0x8B, 0xF9, 0x75, 0, 0x8A, 0x47, 0x2A, 0x3A, 0x05, 0, 0, 0, 0, 0x75, 0, 
					0x8B, 0x47, 0x24, 0x8B, 0xC8, 0xC1, 0xE9, 0x08, 0xF6, 0xC1, 0x01, 0x74, 0, 0xC1, 0xE8, 0x12, 0xA8, 0x01, 0x75, 0, 0xE8, 0, 0, 0, 0, 0xE8, 0, 0, 0, 0 }; // Pattern valid as of 2.1.7
				uint num = this.Process.FindPattern(bPattern, "xx????xxxxx?xxxxx????x?xxxxxxxxxxxx?xxxxxx?x????x????");
				if (num != (uint)this.Process.MainModule.BaseAddress)
				{
